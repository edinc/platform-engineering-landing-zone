...
|------------|----------|-------|
| Platform spoke network | `azurerm_virtual_network`, subnets, NSGs, optional UDR | UDR points to the connectivity & egress firewall when `firewall_private_ip_address` is set. |
...
| GitOps | `azurerm_kubernetes_cluster_extension`, `azurerm_kubernetes_flux_configuration` | Optional GitOps platform Microsoft-managed Flux extension and one root Kustomization per environment. |
...
| TechDocs | `azurerm_storage_account.techdocs` | Optional developer portal Azure Blob publisher storage with private endpoint, shared keys disabled, and container-level RBAC for Backstage plus CI publishers. |
...
| Private Endpoints | `azurerm_private_endpoint` | ACR, Key Vault, and Service Bus when connectivity & egress Private DNS zone IDs are supplied. |

## Deliberate initial-slice boundaries

- AKS Backup, ACR Tasks private agent pools, Postgres CMK, Front Door Private
  Link origin, ingress-nginx, and smoke-test apps are documented platform shared services
...
- AMPLS is owned by connectivity & egress. This stack consumes the Log Analytics
  workspace ID for integrations only.

## State backend

State lives in the Azure foundation account, container `platform`, with a key such as
...
| `private_dns_zone_subscription_id` | Subscription that owns the supplied Private DNS zones; set to the connectivity & egress subscription for centralized zones. |
| `firewall_private_ip_address` | Enables platform subnet UDRs to connectivity & egress Firewall. Required for non-demo AKS and ACA egress; demo keeps the cost-conscious no-Firewall path. |
...
## Developer portal Backstage and TechDocs inputs

Backstage is deployed by a dedicated Flux configuration so GitOps platform reconciliation and observability, SRE & FinOps integrations
can remain enabled without deploying the portal. Set `enable_backstage = true`
after the supply chain & CI/CD-published Backstage image and chart have been promoted. The
...
| 4 | Front Door + ingress-nginx PLS | Front Door Premium + WAF shell implemented; ingress-nginx is seeded for Flux in GitOps platform and PLS origin remains a later hardening step. |
| 5 | Cluster-state repo | Implemented in sibling `../cluster-state-repo/` composition; GitOps platform adds the Flux root configuration contract. |
...
| 8 | DR matrix | Added in `docs/runbooks/dr-matrix.md`; reliability operations validates drills. |
