# Cluster-state repository (platform shared services)
...
| Branch protection | `github_branch_protection` with PR review requirement; status checks are opt-in until supply chain & CI/CD workflows exist |
...
flow through PRs rather than direct Terraform commits. GitOps platform promotes
...
- Azure foundation state backend container `platform` already exists.
...
Tenancy vending and later developer portal/golden-path capabilities write manifests into this
repository. The tenancy vending GitHub App (`platform-vending-bot`) becomes the
