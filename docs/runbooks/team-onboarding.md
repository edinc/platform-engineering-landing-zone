...
| Requirement | Purpose |
| --- | --- |
| Stage 03 identity stack | Provides the Entra tenant and group conventions. |
| Stage 05 namespace vending | Creates workload identities, namespace RBAC, and cluster-state PRs. |
| Stage 09 Backstage | Hosts the `onboard-team` template and syncs Entra groups. |
...
6. A platform admin updates Backstage RBAC configuration by adding the new group
   ...
   `backstage_application_team_group_map_json`, then re-applies the Stage 09
...
| Backstage RBAC map | Platform Terraform/Flux substitutes the group refs and group-to-team map into `backstage-rbac-groups`. | Re-apply the Stage 09 Backstage configuration after adding the group ref and map entry. |
...
| Cost showback | Tags and group-to-team mappings are generated from the same team slug. | Re-run Stage 08 cost allocation after tags appear in Cost Management exports. |
...
Before enabling Stage 11 golden paths for a tenant, run:
...
script to perform read-only Azure checks against deployed Stage 09/10
prerequisites.
